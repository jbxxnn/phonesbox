        ...
        <div className="bg-white rounded-none md:rounded-xl p-6 space-y-6 shadow-none md:shadow-sm">
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 -mx-2 px-2 snap-x">
                {trendingPhones.map((phone, idx) => (
                    <div
                        key={phone.id}
                        className="min-w-[200px] w-[200px] md:min-w-[220px] md:w-[220px] flex-none snap-start"
                    >
                        <Link href={`/phones/${phone.id}`} className="block group relative h-full">
                            {/* Rank Badge */}
                            ...
                            {/* Heart Button */}
                            <button
                                className="absolute top-0 right-0 z-20 p-2 text-slate-400 hover:text-blue-600 transition-colors bg-white/50 backdrop-blur-sm rounded-full m-1"
                                onClick={(e) => {
                                    e.preventDefault();
